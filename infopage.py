...
def data():
    ...
    try:
        results["failed_count"] = int(r.Connection.get(
            "FAILED:" + redis_bucket + ":COUNT").decode('utf8'))
    except:
        results["failed_count"] = 0
    try:
        results["completed"] = int(r.Connection.get(
            "COMPLETED:" + redis_bucket + ":COUNT").decode('utf8'))
    except:
        results["completed"] = 0
